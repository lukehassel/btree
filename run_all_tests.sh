...
# This script runs all tests including basic, memory leak, and advanced tests
...
main() {
    ...
    # Change to tests directory for compilation
    cd tests
    ...
    # Test 2: Advanced Memory Leak Tests
    print_section "🧠 Advanced Memory Leak Tests"
    
    if compile_test "Advanced Tests" "btree_advanced_tests.c test_utils.c ../btree.c ../btree_viz.c" "btree_advanced_tests"; then
        if run_test_suite "Advanced Tests" "btree_advanced_tests" "Running comprehensive memory leak tests..."; then
            test_results+=("Advanced Tests: ✅ PASSED")
        else
            test_results+=("Advanced Tests: ❌ FAILED")
            all_tests_passed=false
        fi
    else
        test_results+=("Advanced Tests: ❌ COMPILATION FAILED")
        all_tests_passed=false
    fi
    ...
    # Test 4: Valgrind Analysis (if available)
    ...
    if command_exists valgrind; then
        ...
        # Run Valgrind on advanced tests
        if [ -f "btree_advanced_tests" ]; then
            run_valgrind_analysis "Advanced Tests" "btree_advanced_tests" "valgrind_advanced.log"
        fi
        ...
        print_status $CYAN "  - tests/valgrind_basic.log"
        print_status $CYAN "  - tests/valgrind_advanced.log"
        print_status $CYAN "  - tests/valgrind_race.log"
    else
    ...
    if [ "$all_tests_passed" = true ]; then
        ...
        print_status $CYAN "  - Run individual test suites: ./tests/run_advanced_tests.sh"
        print_status $CYAN "  - Run with Valgrind: ./tests/run_advanced_valgrind.sh"
        exit 0
    else
